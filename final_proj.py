...
datastuff = []
with open("GBplaces.csv", "r") as filestream:
        for line in filestream:
            currentline = line.split(",")
            datastuff.append(currentline)

# print(datastuff[i]) is a test to see how data is stored
#print(datastuff[1])
#print(datastuff[2])

# need arrays containing arrays of [population, latitude and longitude] for each place
organised_data = []
for i in range(1,101):
    temp_array = [ float(datastuff[i][2]), float(datastuff[i][3]), float(datastuff[i][4])]
    organised_data.append(temp_array)

# calculate average population just to use as an arbitrary weighting of the importance of proximity to the port
totl = 0
for i in range(0,100):
    totl += organised_data[i][0]
ave_pop = totl/100
...
print("Differential equations calculation commencing...")
...
while True:
    try:
        print("You may select the rates of infection and recovery below")
        print("The values must both be between 0 and 1, if not the default values will be set")
        print("Default is:")
        print("\u03B2 [1/days] = 0.3")
        print("\u03B3 [1/days] = 0.1")
        beta = float(input("Enter the rate of infection, \u03B2 , to be used in model: "))
        gamma = float(input("Enter the rate of recovery, \u03B3 , to be used in model: "))
        break
    except ValueError:
...
print("The peak of the disease will be reached on day " + str(which_array) + ", at which point " + str(int(round(I_max))) + " people in Great Britain will be infected")
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
